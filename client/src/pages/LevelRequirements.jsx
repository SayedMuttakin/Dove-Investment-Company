...
const LevelRequirements = () => {
    ...
    return (
        <div className="min-h-screen bg-dark-300 pb-24">
            {/* Header */}
            <div className="fixed top-0 left-0 right-0 z-50 bg-dark-300/80 backdrop-blur-xl border-b border-white/5">
                <div className="max-w-md mx-auto px-4 h-16 flex items-center justify-between">
                    ...
                    <h1 className="text-white font-black text-lg uppercase tracking-tight">Level Status</h1>
            ...
            <div className="max-w-md mx-auto px-4 pt-20 space-y-6">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <h2 className="text-white font-bold text-lg">Level Requirements</h2>
                </div>
                            ...
                            <div key={`${level.from}-${level.to}`} className="relative group">
                                ...
                                <div className="relative">
                                    <img
                                        ...
                                        className={`w-full h-32 rounded-t-[2rem] border-2 border-b-0 transition-all duration-300 object-cover ${isCurrentLevel ? 'border-primary' : (currentLevel >= level.from ? 'border-white/30' : 'border-white/10')
                                            }`}
                                    />
                                    {isCurrentLevel && (
                                        <div className="absolute top-0 right-0 bg-primary text-black text-[10px] font-black px-4 py-1.5 rounded-bl-[1.5rem] shadow-lg z-40 border-b border-l border-white/20 uppercase">
                                            ACTIVE
                                        </div>
                                    )}
                                ...
                                {/* Level Stats Panel */}
                                <div className={`bg-dark-200 border-2 border-t-0 rounded-b-[2rem] p-5 relative z-0 ${isCurrentLevel ? 'border-primary shadow-[0_20px_40px_rgba(164,241,58,0.05)]' : 'border-white/10'
                                    }`}>
                                    <div className="flex items-center gap-2 mb-5">
                                        <span className="text-white font-black text-xl italic uppercase tracking-tighter">Level {level.from + 1}</span>
                                        {!isCurrentLevel && <Shield size={16} className="text-white/10" />}
                                    </div>

                                    {isCurrentLevel && (
                                        <div className="mb-6">
                                            <div className="flex justify-between text-[11px] font-black text-white/40 mb-2 uppercase tracking-widest">
                                                <span>PROGRESS</span>
